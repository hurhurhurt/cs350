...
int getopt_new PARMS ((int argc, char **argv, char *options));
...
argv[0] = myname;                /* for use by getopt_new */
...
/* table to find out which characters are binary */
   ...
   while ((option = getopt_new (argc, argv, "umhvtsbz")) != EOF) {
      switch (option) {
...
#define  TEMPLATE    "XXXXXX"
   {
      ...
      strcat (tfname, mkdtemp (template));
   }
